...
1. **Limpeza de logs** — remove logs/relatórios mais antigos que
   `KPI_LOG_RETENTION_DAYS` (padrão 5 dias).
...
3. **Etapa 1 — Coleta Git** (`executar_coleta_git`) — percorre `config.REPOS`
   ...
   `KPI_SINCE_DAYS` dias e grava o consolidado em `gitlab_git_data.json`.
...
| `--full` / `KPI_REFRESH_MODE=full` | Reprocessa todos os metadados/enriquecimentos. |
| `--all-modules` / `KPI_ALL_MODULES=1` | Inclui issues de qualquer módulo (padrão). |
| `--initial-load` / `KPI_INITIAL_LOAD=1` | Carga histórica: não exclui fechadas antigas (a data de corte continua valendo). |
| `KPI_FAST_REPO_SYNC=1` | Desliga detectores Git (Área/Tipo/Dev) — usa só título/labels. |

## Filtros aplicados antes do sync

`sync_supabase._filter_issues_for_sync` aplica, na ordem:

1. **Fechadas antigas** — issues `closed` há mais de `KPI_CLOSED_EXCLUDE_DAYS`
